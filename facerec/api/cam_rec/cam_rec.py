    ...
    def recognize(self):
        ...
        while True:
            ...
            for (top, right, bottom, left), unknown_face_encoding in zip(face_locations, unknown_face_encodings):
                ...
                matches = face_recognition.compare_faces(list(self.known_face_encodings.values()), unknown_face_encoding)
                name = "Unknown"

                # If a match was found in known_face_encodings, just use the first one.
                if True in matches:
                    first_match_index = matches.index(True)
                    name = list(self.known_face_encodings.keys())[first_match_index]
        ...
        while True:
            ...
            if process_this_frame:
                ...
                for face_encoding in face_encodings:
                    # See if the face is a match for the known face(s)
                    matches = face_recognition.compare_faces(list(self.known_face_encodings.values()), face_encoding)
                    name = "Unknown"
                    ...
                    # Or instead, use the known face with the smallest distance to the new face
                    face_distances = face_recognition.face_distance(list(self.known_face_encodings.values()), face_encoding)
                    best_match_index = np.argmin(face_distances)
                    if matches[best_match_index]:
                        name = list(self.known_face_encodings.keys())[best_match_index]
...
def test():
    camRecer_basic = cam_Recognizer_basic()
    camRecer_basic.recognize()
    # camRecer_faster = cam_Recognizer_faster()
    # camRecer_faster.recognize()
